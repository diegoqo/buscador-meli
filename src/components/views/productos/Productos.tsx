...
const Productos = () => {
    ...
    }, [query, response, errorPeticion, loading]);
    ...
    return (
        <Box className={'productos'}>
            ...
            {R.pathOr([], ['items'], results).length > 0 &&
                <>
                    {R.pathOr([], ['categories'], results).length > 0 &&
                        <Box className={'productos__container-miga'}>
                            <MigaDePan
                                categorias={R.pathOr([], ['categories'], results)}/>
                        </Box>}
                    {R.pathOr([], ['items'], results).slice(0, 4).map((it: IItem) =>
                        <>
                            <Paper className={'productos__container-grilla'}
                                   key={`${it.id}-paper`}
                            >
                                <a className={'productos__container-grilla--href'} onClick={() => handleDetail(it.id)}
                                   key={`${it.id}-a`}>
                                    <Grid container spacing={2} key={`${it.id}-grid-img-container`}>
                                        <Grid item className={'productos__container-img-producto'}
                                              key={`${it.id}-grid-img`}>
                                            <Img alt="complex" src={it.picture} key={`${it.id}-img`}/>
                                        </Grid>
                                        <Grid item xs={12} sm container className={'productos__container-desc-prod'}
            ...
            {
                errorPeticion && !loading &&
                <Box>
                    <span>Ocurrio un error consultando tu producto, por favor intenta mas tarde.</span>
                </Box>
            }
        </Box>
    );
